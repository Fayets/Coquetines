...
export default function RankingProductosVendidos() {
  ...
  const totalPages = Math.max(1, Math.ceil(rows.length / RANKING_PER_PAGE));
  const currentPageSafe = Math.min(Math.max(1, currentPage), totalPages);
  const rowsPagina = rows.slice(
    (currentPageSafe - 1) * RANKING_PER_PAGE,
    currentPageSafe * RANKING_PER_PAGE
  );
        ...
        {loading ? (
          ...
          <div className="flex items-center justify-between mt-4">
            <p className="text-sm text-slate-500">
              Página {currentPageSafe} de {totalPages} · {rows.length} producto{rows.length !== 1 ? "s" : ""}
            </p>
