...
export default function NuevoCredito() {
  ...
  useEffect(() => {
    const fetchProductos = async () => {
      try {
        if (esOwner) {
          if (sucursalOwnerId === "" || Number.isNaN(Number(sucursalOwnerId))) {
            setProductos([]);
            return;
          }
        }
        const sid = esOwner ? Number(sucursalOwnerId) : getSucursalId();
        const url =
  ...
  const handleSubmit = async (e) => {
    ...
    if (esOwner && (sucursalOwnerId === "" || Number.isNaN(Number(sucursalOwnerId)))) {
      Swal.fire({ icon: "warning", title: "Sucursal", text: "Seleccioná la sucursal del crédito." });
      return;
    }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      {esOwner && (
        <div className="mb-6 bg-white rounded-xl border border-slate-200 shadow-sm p-4 sm:p-6 max-w-xl">
          <label className="block text-sm font-medium text-slate-700 mb-1">Sucursal del crédito</label>
          <select
          ...
          <p className="text-xs text-slate-500 mt-2">
            Los productos y el stock se toman de la sucursal elegida.
          </p>
        </div>
      )}
