...
function formatFecha(fechaStr) {
  if (!fechaStr) return "";
  const d = typeof fechaStr === "string" ? new Date(fechaStr + "T12:00:00") : new Date(fechaStr);
...
export default function CajaDiaria() {
  ...
  const fetchResumen = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_URL}/caja/resumen`, {
        params: sid != null ? { fecha, sucursal_id: sid } : { fecha },
      ...
      if (err.response?.status === 404) {
        ...
        const esCajaInexistente =
          typeof detail === "string" && detail.toLowerCase().includes("no existe caja");
  ...
  const handleAbrirCaja = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${API_URL}/caja/abrir`,
        { sucursal_id: sid ?? undefined, saldo_inicial: saldo, fecha: fecha || undefined },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.data?.success) {
  ...
  const handleCerrarCaja = async () => {
    ...
    try {
      ...
      let filename = `cierre_caja_${fecha}.pdf`;
      if (disposition) {
  ...
  const handleEgresoManual = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        ...
        { params: sid != null ? { sucursal_id: sid } : {}, headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.data?.success) {
    ...
    <div className="p-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Ingresos y egresos del día</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 text-center">
          ...
          <p className="text-slate-600 mb-2">No hay caja abierta para esta fecha.</p>
          <p className="text-slate-500 text-sm mb-6">Abrí la caja para comenzar a registrar movimientos.</p>
          <button
            type="button"
                ...
                {resumen.movimientos?.length > 0 ? (
                  resumen.movimientos.map((m) => (
                    <tr key={m.id}>
                      ...
                      <td className="text-slate-600">{m.descripcion || "—"}</td>
      ...
      {modalAbrir && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
          <div className="bg-white rounded-xl shadow-lg max-w-md w-full p-6">
            ...
            <p className="text-sm text-slate-500 mb-1">Saldo inicial en caja para el día {fecha}</p>
