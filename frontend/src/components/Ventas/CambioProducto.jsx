...
export default function CambioProducto() {
  ...
  /** Sucursal cuyas ventas se listan (puede no ser la de operación). */
  const [sucursalVentaFiltro, setSucursalVentaFiltro] = useState(() => {
    if (user?.role === "OWNER") return "";
    const sid = getSucursalId();
    return sid != null ? String(sid) : "";
  });
  /** Sucursal de stock/caja (solo la dueña elige otra; el resto queda anclado al usuario en backend). */
  const [sucursalOperacion, setSucursalOperacion] = useState(() => {
    if (user?.role === "OWNER") return "";
    const sid = getSucursalId();
    return sid != null ? String(sid) : "";
  });
  ...
  const sidVenta =
    sucursalVentaFiltro !== "" ? Number(sucursalVentaFiltro) : esOwner ? null : getSucursalId();
  const sidOperacion =
    esOwner
      ? sucursalOperacion !== ""
        ? Number(sucursalOperacion)
        : null
      : getSucursalId();

  /** Opciones del selector «Registrar cambio»: dueña elige; el resto solo su sucursal (con fallback si falta en la lista). */
  const sucursalesOpciones = useMemo(() => {
    const list = Array.isArray(sucursales) ? [...sucursales] : [];
    if (esOwner) return list;
    const sid = getSucursalId();
    if (sid != null && !list.some((s) => Number(s.id) === sid)) {
      list.push({ id: sid, nombre: "Tu sucursal" });
    }
    return list;
  }, [sucursales, esOwner]);

  /** Sucursales para «Venta en sucursal»: todas las activas + la elegida si aún no cargó la lista. */
  const sucursalesListadoVenta = useMemo(() => {
    const list = Array.isArray(sucursales) ? [...sucursales] : [];
    const want =
      sucursalVentaFiltro !== "" && Number.isFinite(Number(sucursalVentaFiltro))
        ? Number(sucursalVentaFiltro)
        : null;
    if (want != null && !list.some((s) => Number(s.id) === want)) {
      list.push({ id: want, nombre: `Sucursal #${want}` });
    }
    return list;
  }, [sucursales, sucursalVentaFiltro]);

  /** Query GET /ventas/all solo para esta pantalla: admin/empleado pueden listar otra sucursal (venta en Centro, cambio en Trento). */
  const queryVentasListadoCambio = (sid) => {
    const p = new URLSearchParams();
    p.set("listado_cambio", "true");
    if (sid != null && Number.isFinite(Number(sid))) {
      p.set("sucursal_id", String(sid));
    }
    return `?${p.toString()}`;
  };
  ...
  useEffect(() => {
    if (!token) return;
    if (esOwner && !sucursalVentaFiltro) {
      setVentas([]);
      setVentaSel(null);
      setBloquesCambio([]);
      return;
    ...
    const q = queryVentasListadoCambio(sidVenta);
    axios
  ...
  }, [token, esOwner, sucursalVentaFiltro, sidVenta]);

  useEffect(() => {
    if (!token || sidOperacion == null) {
      setProductos([]);
      return;
    }
    const url = `${API_URL}/products/all?sucursal_id=${sidOperacion}`;
    axios
  ...
  useEffect(() => {
    setPageVentas(1);
  }, [modoFechaVenta, fechaDiaExacta, fechaDesde, fechaHasta, buscarVenta, sucursalVentaFiltro, sidVenta]);
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (esOwner && (!sucursalVentaFiltro || !sucursalOperacion)) {
      Swal.fire(
        "Sucursal",
        "Elegí la sucursal de la venta y la sucursal donde se registra el cambio (stock y caja).",
        "warning"
      );
      return;
    }
    ...
    if (esOwner) body.sucursal_id = Number(sucursalOperacion);
    ...
    try {
      ...
      setVentas(Array.isArray(vr.data) ? vr.data : []);
      const pr = await axios.get(`${API_URL}/products/all?sucursal_id=${sidOperacion}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-semibold text-slate-900 flex items-center gap-2 mb-6">
      ...
      <form onSubmit={submit}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            ...
            <select
              required
              value={sucursalVentaFiltro}
              onChange={(e) => {
                const v = e.target.value;
                setSucursalVentaFiltro(v);
                if (esOwner) {
                  setSucursalOperacion(v);
                }
                setVentaSel(null);
                setBloquesCambio([]);
                setBuscarLinea("");
            ...
            >
              {esOwner ? <option value="">Elegí sucursal…</option> : null}
              {sucursalesListadoVenta.map((s) => (
            ...
            <p className="text-xs text-slate-500 mt-1.5">
              Sucursal donde se hizo la venta. Podés elegir otra (ej. compró en Centro y el cambio lo hacés acá).
            </p>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            <label className="flex items-center gap-2 text-sm font-medium text-slate-700 mb-1">
              <Building2 className="h-4 w-4" />
              Registrar cambio en (stock y caja)
            </label>
            <select
              required={esOwner}
              value={sucursalOperacion}
              disabled={!esOwner}
              onChange={(e) => {
                setSucursalOperacion(e.target.value);
                setBloquesCambio([]);
                setBuscarProd("");
              }}
              className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 bg-white disabled:bg-slate-50 disabled:text-slate-600"
            >
              {esOwner ? <option value="">Elegí sucursal…</option> : null}
            ...
            <p className="text-xs text-slate-500 mt-1.5">
              {esOwner
                ? "Productos de reemplazo y movimientos de stock en esta sucursal."
                : "Reemplazo y movimientos de stock/caja en tu sucursal (donde atendés el cambio)."}
            </p>
          </div>
        </div>
            ...
            {loadingVentas ? (
              ...
              <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 bg-white"
                        disabled={esOwner && !sucursalVentaFiltro}
                      />
                    </div>
                ...
                <div className="border border-slate-200 rounded-lg overflow-hidden">
                  {esOwner && !sucursalVentaFiltro ? (
                    <p className="p-4 text-sm text-slate-500">Elegí la sucursal de la venta para listar.</p>
            ...
            <div className="overflow-auto max-h-[380px] flex-1">
              {sidOperacion == null ? (
                <p className="p-4 text-slate-500 text-sm">
                  {esOwner
                    ? "Elegí en qué sucursal se registra el cambio (stock y caja) o esperá la carga."
                    : "Esperá la carga de productos."}
                </p>
        ...
        <div className="mt-6 rounded-xl border-2 border-slate-200 bg-slate-50 overflow-hidden">
          ...
          <div className="px-4 py-4 border-t border-slate-200 bg-white flex justify-end">
            <button
              type="submit"
              disabled={loading || sidOperacion == null || (esOwner && !sucursalVentaFiltro)}
