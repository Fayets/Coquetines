...
const EditarCliente = () => {
  ...
  useEffect(() => {
    const fetchClienteData = async () => {
      ...
      try {
        const response = await axios.get(`${API_URL}/clientes/get_by_dni/${dni}`,
          {
            headers: {
    ...
    fetchClienteData();
  }, [dni]); // Solo se ejecuta cuando el componente se monta o el ID cambia
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${API_URL}/clientes/update/${cliente.id}`, // Usamos cliente.id
        cliente, 
        {
          headers: {
