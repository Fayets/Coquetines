...
from src.controllers.auth_controller import (
    ...
    get_sucursal_id_for_ventas_cambio_listado,
)
...
def get_all_ventas(
    ...
    listado_cambio: bool = Query(
        False,
        description="Si true, admin/empleado pueden filtrar ventas por sucursal_id (pantalla cambio de producto).",
    ),
    current_user=Depends(get_current_user),
):
    if listado_cambio:
        sid = get_sucursal_id_for_ventas_cambio_listado(current_user, sucursal_id)
    else:
...
def registrar_cambio_producto(
    ...
    """
    Cambio de producto: stock y caja en la sucursal de operación (`sucursal_id` resuelta por el usuario).
    La venta puede ser de otra sucursal: el ingreso por devolución va al stock equivalente en la sucursal de operación.
    """
...
def registrar_cambio_producto_lote(
    ...
    """Varios reemplazos en la misma venta. Misma regla de sucursal que POST /cambios/registrar."""
    sid = get_sucursal_id_for_user(current_user, body.sucursal_id)
    if sid is None:
        raise HTTPException(
            status_code=400,
...
def listado_cambios_venta(
    ...
    todas: bool = Query(
        False,
        description="Solo OWNER: devuelve cambios de todas las sucursales (sin filtrar por sucursal_id).",
    ),
    ...
    if todas:
        if role != "OWNER":
            raise HTTPException(status_code=403, detail="Solo la dueña puede listar todos los cambios.")
        try:
            return cambios_service.listar_cambios(None, limit=limit)
        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))
