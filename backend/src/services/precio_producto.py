...
def metodo_para_precio_linea(venta_producto, venta) -> str:
    """
    Método de cotización para una línea de venta (p. ej. cambios de producto).
    Prioriza VentaProducto.tipo_precio; si falta, Venta.metodo_pago si es un solo medio legible.
    """
    tp = getattr(venta_producto, "tipo_precio", None)
    if tp is not None and str(tp).strip():
        return str(tp).strip()
    vm = getattr(venta, "metodo_pago", None) or ""
    vm = str(vm).strip()
    if not vm or " + " in vm or vm.lower().startswith("mixto"):
        return "Efectivo"
    return vm
