...
class Venta(db.Entity):
    ...
    # Resumen legible (p. ej. un medio o "Efectivo + Débito"); el cobro detallado está en VentaPago.
...
class VentaPago(db.Entity):
    """Fracciones de cobro de una venta (pago mixto)."""
    id = PrimaryKey(int, auto=True)
    venta = Required("Venta", column="venta_id")
    metodo_pago = Required(str)
    monto = Required(float)
    _table_ = "Ventas_Pagos"
...
class VentaProducto(db.Entity):
    ...
    # Método usado solo para cotizar la línea (Efectivo, Transferencia, etc.); independiente del cobro final.
    tipo_precio = Optional(str, nullable=True)
...
class CajaDiaria(db.Entity):
    """Caja diaria por sucursal: dos turnos por día (MAÑANA / TARDE), cada uno con ciclo propio."""
    id = PrimaryKey(int, auto=True)
    sucursal = Optional(Sucursal, column="sucursal_id")
    fecha = Required(date)
    turno = Required(str, default=TurnoCaja.MANANA.value)  # MAÑANA | TARDE
    composite_key(sucursal, fecha, turno)
...
class MovimientoCaja(db.Entity):
    ...
    # True si la venta se cobró con más de un medio (un solo movimiento de ingreso agrega el total).
    pago_mixto = Required(bool, default=False)
